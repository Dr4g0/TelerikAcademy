...
namespace GsmInfo
{
    class GSMTest
    {
        static void Main()
        {
            ...
            GSM[] myGsm = new GSM[2];
            Battery samsungBattery = new Battery("Samsung", 35, 11);
            Display samsungDisplay = new Display();
            myGsm[0] = new GSM("Galaxy SII", "Samsung",450,"Filcho Filchev",samsungBattery,samsungDisplay);
            Battery nokiaBattery=new Battery("Nokia",15,25,BatteryType.NiCD);
            Display nokiaDisplay=new Display(4,24000);
            myGsm[1] = new GSM("E52","Nokia",200,"Kalitko Kalitarov");
            for (int i = 0; i < myGsm.Length; i++)
            {
                Console.WriteLine("{0} {1} {2}",myGsm[i].Manufacturer,myGsm[i].Model,myGsm[i].ToString());
            }
            Console.WriteLine("{0} {1} {2}",GSM.IPhone4S.Model, GSM.IPhone4S.Manufacturer, GSM.IPhone4S.ToString());
